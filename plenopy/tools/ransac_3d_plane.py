...
def draw_sample(xyz_point_cloud, sample_size, prng):
    sample = prng.choice(
        np.arange(xyz_point_cloud.shape[0]), size=sample_size, replace=False
    )
...
def fit(
    ...
    for i in range(max_number_itarations):
        ...
        distances_to_plane = distance_to_plane(
            plane_model=model_based_on_sample, xyz_point_cloud=xyz_point_cloud
        )
