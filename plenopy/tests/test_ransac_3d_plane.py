...
def test_estimate_3d_plane_model_1():
    ...
    m = pl.tools.ransac_3d_plane.estimate_3d_plane_model(
        xyz_point_cloud=xyz)
...
def test_estimate_3d_plane_model_3():
    ...
    x = xx.flatten()
    y = yy.flatten()
    z = np.random.normal(loc=z0, size=400)
    xyz = np.c_[x, y, z]
    ...
    assert np.isclose(d, z0, atol=5e-2)
...
def test_draw_sample():
    ...
    s = pl.tools.ransac_3d_plane.draw_sample(np.arange(100), 100)
        ...
        for ny in np.linspace(-1, 1, 4):
            for nz in np.linspace(-1, 1, 4):
                for d in np.linspace(1, 2, 4):
                    ...
                    n_points_plane = 70
                    x = np.random.uniform(
                        low=-10, high=10, size=n_points_plane)
                    y = np.random.uniform(
                        low=-10, high=10, size=n_points_plane)
                    ...
                    assert np.sum(inliers[0: n_points_plane]) >= n_points_plane
                    assert (
