...
def event_header_repr(h):
    ...
    out += (
        "  7 "
        + "z coordinate (height) of first interaction "
        + str(h[7 - 1])
        + " cm\n"
    )
    out += (
        " 11 " + "zenith angle Theta " + str(np.rad2deg(h[11 - 1])) + " deg\n"
    )
    out += (
        " 12 " + "azimuth angle Phi " + str(np.rad2deg(h[12 - 1])) + " deg\n"
    )
...
def run_header_repr(h):
    ...
    out += (
        "249 "
        + "YSCATT scatter range in x direction for Cherenkov "
        + str(h[249 - 1])
        + " cm\n"
    )
    return out
