...
def integrate_around_arrival_peak(sequence, integration_radius=1):
    """
...
def pixel_sequence(lixel_sequence, number_pixel, number_paxel):
    return _to_image_sequence(
        lixel_sequence, number_pixel, number_paxel, axis=1
    )


def paxel_sequence(lixel_sequence, number_pixel, number_paxel):
    return _to_image_sequence(
        lixel_sequence, number_pixel, number_paxel, axis=0
    )
...
def _to_image_sequence(lixel_sequence, number_pixel, number_paxel, axis):
    ...
    imgs = np.zeros(shape=(number_time_slices, bins), dtype=np.uint16)
    for t in range(number_time_slices):
        imgs[t, :] = (
            lixel_sequence[t, :]
            .reshape(number_pixel, number_paxel)
            .sum(axis=axis)
        )
    return imgs
