...
def corsika_event_header_repr(h):
    ...
    out+= '  7 '+'z coordinate (height) of first interaction '+str(h[7-1])+' cm\n'
    out+= ' 11 '+'zenith angle Theta '+str(np.rad2deg(h[11-1]))+' deg\n'
    out+= ' 12 '+'azimuth angle Phi '+str(np.rad2deg(h[12-1]))+' deg\n'
    out+= ' 98 '+'number of uses '+str(int(h[98-1]))+'\n'
    for i in range(int(h[98-1])):
        out+= '    '+'reuse '+str(i+1)+': core position x='+str(h[98-1+i+1])+' cm, y='+str(h[118-1+i+1])+' cm\n'    
    return out
...
def corsika_run_header_repr(h):
    ...
    out+= '248 '+'XSCATT scatter range in x direction for Cherenkov '+str(h[248-1])+' cm\n'
    out+= '249 '+'YSCATT scatter range in x direction for Cherenkov '+str(h[249-1])+' cm\n'
    return out
