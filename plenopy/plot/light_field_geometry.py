...
def efficieny_relative_error_hist(lss, ax):
    rel_error = (
        lss.efficiency_std[lss.efficiency > 0.0]
        / lss.efficiency[lss.efficiency > 0.0]
    )
...
def c_vs_c_std(lss, ax):
    ...
    hist_2d(
        c_mean[valid], c_std[valid], ax, norm=colors.PowerNorm(gamma=1 / 2)
    )
...
def x_y_hist2d(lss, ax):
    ...
    hist_2d(
        x_pos, y_pos, ax, aspect="equal", norm=colors.PowerNorm(gamma=1 / 2)
    )
    ax.set_xlabel(r"$\overline{x}$/m")
...
def cx_cy_hist2d(lss, ax):
    ...
    hist_2d(
        cx_pos, cy_pos, ax, aspect="equal", norm=colors.PowerNorm(gamma=1 / 2)
    )
    ax.set_xlabel(r"$\overline{c_x}$/$^\circ$")
...
class PlotLightFieldGeometry(object):
    ...
    def save_sensor_plane_overview(
        ...
        if indicate_zoom_region_center:
            splt.ax_add_box(
                ax, xlim=zoom_center_x, ylim=zoom_center_y, linewidth=1
            )
        if indicate_zoom_region_pos_x:
            splt.ax_add_box(
                ax, xlim=zoom_posx_x, ylim=zoom_posx_y, linewidth=1
            )
        if indicate_zoom_region_pos_y:
            splt.ax_add_box(
                ax, xlim=zoom_posy_x, ylim=zoom_posy_y, linewidth=1
            )
