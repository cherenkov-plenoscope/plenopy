...
def tomographic_point_spread_function_sparse(rays, binning, show_progress=False):
    ...
    for ray_id in range(x_bins.shape[0]):
        x_ = x_bins[ray_id]
        y_ = y_bins[ray_id]
        z_ = np.arange(len(x_))

        good = (x_ >= 0) | (x_ < binning.number_xy_bins)
        good |= (y_ >= 0) | (y_ < binning.number_xy_bins)

        voxel_indices = np.ravel_multi_index(
            [x_[good], y_[good], z_[good]],
            dims=binning.dims,
            order='F'
        )
    ...
    psf = coo_matrix(
        (np.ones_like(voxel_ids), (voxel_ids, ray_ids)),
